...
@property (strong, nonatomic) IBOutlet UILabel *colorDialogLabel;

@property NSString *response,*labelColor;
@end

int score=0,times=0,number=0,flagForVoiceInput=0;
BOOL flagTest=false,flagInst=true;
NSString *answer;
NSArray *colors = @[@"GREEN", @"BROWN", @"RED",@"BLUE"];
NSArray *seq ;
...
-(void) instructions
{
    ...
    [self speakText:@"Hello !!! Welcome to Stroop Color Dialog Test."];
    [self speakText:@"To perform the test, call out the color of the word printed on the screen. For example"];
    _colorDialogLabel.text=@" ";
    _colorDialogLabel.backgroundColor=[UIColor greenColor];
...
-(void) imageAssignment:(BOOL) labelAssignment
{
    ...
    if(labelAssignment)
    {
        ...
        _ColorLabel.text=[NSString stringWithFormat:@"%d",(label+1)];
        ...
        switch (label) {
            case 0:
                ...
                break;
            case 1:
                _colorDialogLabel.backgroundColor=[UIColor brownColor];
                break;
            case 2:
                _colorDialogLabel.backgroundColor=[UIColor redColor];
                break;
            case 3:
                _colorDialogLabel.backgroundColor=[UIColor blueColor];
                break;
            default:
                break;
        }
        answer=[answer stringByAppendingString:colors[number]];
        number++;
    }
    else
    {
...
- (IBAction)tappedOnRecord:(id)sender {
    if(!_synthesizer.speaking)
    {
        if (transactionState == TS_RECORDING) {
            ...
            detectionType = SKNoEndOfSpeechDetection;//SKLongEndOfSpeechDetection; /* Dictations tend to be long utterances that may include short pauses. */
...
- (IBAction)tappedOnStart:(id)sender {
    _ColorLabel.text=@"WELCOME TO STROOP COLOR DIALOG TEST";
    _ColorLabel.font=[UIFont fontWithName:@"Optima-ExtraBlack" size:10.0];
    _colorDialogLabel.backgroundColor=[UIColor whiteColor];
...
{
    times++;
    NSArray* res = [response componentsSeparatedByString: @" "];
    NSArray* ans = [answer componentsSeparatedByString: @" "];
    for(int i=0;i<[res count];i++)
    {
        NSString *r=[NSString stringWithString:res[i]];
        NSString *a=[NSString stringWithString:ans[i]];
        if([r.uppercaseString isEqualToString:a.uppercaseString])
            score++;
    }
...
- (IBAction)tappedOnNext:(id)sender {
    if(!flagTest)
    {
        ...
        {
            [self speakText:@"Now lets test for recall ability, Try recollecting the colors in sequence. Click on Start Button When your ready"];
