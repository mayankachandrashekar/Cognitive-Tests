...
NSArray *words=@[@"Apple ",@" Bear ",@" Piano ",@" Hammer ",@" Banana ",@" Chisel ",@" Trumpet ",@" Orange ",@" Lion",@" Wrench",@" Peach",@" Saw ",@" Drum ",@" Tiger",@" Violin",@" Giraffe"];
...
-(void) wordAssignment
{
    ...
    {
        _ColorLabel.text=[_ColorLabel.text stringByAppendingString:words[i]];
    }
}
...
-(void) checkResults:(NSString*)response
{
    ...
    for(int i=0;i<words.count;i++)
    {
        NSString *word=[NSString stringWithString:words[i]];
        if([response.uppercaseString containsString:word.uppercaseString])
        {
            _ColorLabel.text=[_ColorLabel.text stringByAppendingString:word];
            score++;
        }
    }
...
- (IBAction)tappedOnNext:(id)sender {
    if(!flagTest)
    {
            [self speakText:@"Now lets test for word recall ability, Try recollect the words  displayed.. Click on Start Button When your ready"];
